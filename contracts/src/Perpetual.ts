...
export class Position extends Struct({
  ...
  calcPnl(closePrice: UInt64): Int64 {
    ...
    let posPnl = Provable.if(
      ...
      SCALING_FACTOR.sub(
        Int64.fromUnsigned(closePrice).mul(SCALING_FACTOR).div(this.openPrice)
      ),
      Int64.fromUnsigned(closePrice)
        .mul(SCALING_FACTOR)
        .div(this.openPrice)
        .sub(SCALING_FACTOR)
    );
...
export class Perpetual extends SmartContract {
  ...
  @method openPosition(keyWitness: MerkleMapWitness, position: Position) {
